...
# distributed under the License istensorflow.python.framework.errors_impl.NotFoundError distributed on an "AS IS" BASIS,
...
rnn = tf.load_op_library("OCR/cc/rnn_ops.so")
...
def tflstm_layer(inp,
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        lstm = CudnnLSTM(num_layers=1, num_units=num_nodes)
        lstm.build(inp.get_shape())
