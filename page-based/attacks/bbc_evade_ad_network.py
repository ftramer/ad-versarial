...
def main(argv=None):
    ...
    safe_mkdir("output/bbc_evade_ad_network")
    ...
    mask_h = 150
    mask_w = 920
    mask = tf.Variable(initial_value=np.random.randint(low=-16, high=16, size=(mask_h, mask_w, 3)), dtype=tf.float32)
    ...
    masked_input = tf.clip_by_value(tf.add(inputs, padded_mask), 0, 255)
    ...
    for epoch in range(epochs):
        if epoch % 10 == 0:
            ...
            x0 = [box[0] - 50 for box in boxes]
            y0 = [box[1] + 25 for box in boxes]
            feed_dict = {
                inputs: X_test,
                x_min: x0,
                y_min: y0,
            ...
            for j in range(len(X_test)):
                ...
                img.save("output/bbc_evade_ad_network/img_{}_{}.png".format(epoch, j))

                draw_boxes(filtered_boxes, img, classes, (FLAGS.size, FLAGS.size))
                img.save("output/bbc_evade_ad_network/img_boxes_{}_{}.png".format(epoch, j))
                img.close()
